...
export const BranchableStatement = (node: BranchableNode, context: TContext, getCondition: (branchableNode: BranchableNode) => TCondition): TStatement => {
    ...
    return getStatementByBranchCondition(condition.branchCondition)
        (() => Statement(
            Context(
                context.childInfo,
                withLtlSem5(node.begin)(context)(FormulaStatement(condition.formula, condition.prettyFormula, '0'))
            ),
            []
        ))
        (() => {
            ...
            return Statement(
                Context(
                    contextInfo,
                    withLtlSem5(node.begin)(statement.context)(
...
export const handleIDK = (context: TContext) =>
    ...
    const func = () => Statement(
        Context(iteratorStatement.context.childInfo, withLtlSem5(node.begin)(bodyStatement.context)(formula)),
        iteratorStatement.promelaNodes
    );
    if (promelaBodyNodes.length > 0 || condition.lunaDfs.find(isConstant(childInfo))) {
        const m2 = withLtlSem5(node.begin)(bodyStatement.context)(formula);
        const m3 = withLtlSem6(node.begin)(Context(bodyStatement.context.childInfo, m2))(formula);
...
const withLtlSem5 = (begin: number) => (context: TContext) => (formula: TFormulaStatement): TMetaInfo => {
    const metaInfo = context.metaInfo;
...
const withLtlSem6 = (begin: number) => (context: TContext) => (formula: TFormulaStatement) => {
    const metaInfo = context.metaInfo;
    const contextInfo = context.childInfo;
    const cond = getCond(formula, contextInfo.constants);
